...
2. If no issue is supplied, perform only a read-only search for a likely existing issue (for example, with `gh issue list`). Present any likely candidate for confirmation before using it. Do not guess the repository, silently select an unrelated candidate, or treat inaccessible results as proof that no issue exists.
