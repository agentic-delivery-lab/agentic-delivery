...
assert_contains 'ADR tracking issue'
assert_contains 'sub-issue'
assert_contains 'feature branch'
assert_contains 'never create a pull request from `main`'
assert_contains 'is only the protected base'
...
assert_contains 'broader source issue'
