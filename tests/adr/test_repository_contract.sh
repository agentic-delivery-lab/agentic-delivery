...
assert_file() {
  local path=$1
  if [[ ! -f "$path" ]]; then
    echo "expected this file to exist: $path" >&2
    exit 1
  fi
}
...
assert_contains() {
  ...
  if ! grep -Fq -- "$needle" "$path"; then
    echo "expected this text in $path: $needle" >&2
    exit 1
  fi
}
...
if grep -Eq '^status:' "$DECISIONS_DIR/adr-template.md"; then
  echo "the ADR template must not define a status field" >&2
  exit 1
fi
...
for obsolete_file in \
  ...
  if [[ -e "$obsolete_file" ]]; then
    echo "obsolete ADR acceptance file still exists: $obsolete_file" >&2
    exit 1
  fi
done
...
assert_contains 'Check the dedicated runner' "$SMOKE_WORKFLOW"

printf 'Repository and runner contracts passed\n'
